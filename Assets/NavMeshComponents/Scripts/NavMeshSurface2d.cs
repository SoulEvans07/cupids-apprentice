...
namespace UnityEngine.AI
{
    ...
    public class NavMeshSurface2d : MonoBehaviour
    {
        ...
        Bounds CalculateWorldBounds(List<NavMeshBuildSource> sources)
        {
            ...
            {
                var grid = FindObjectOfType<Grid>();
                var tilemap = grid.GetComponentInChildren<CompositeCollider2D>();
                var bounds = new Bounds(tilemap.bounds.center,new Vector3(tilemap.bounds.size.x, tilemap.bounds.size.y, tilemap.bounds.size.y));
                return bounds;
            }
